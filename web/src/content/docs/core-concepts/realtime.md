...
Zork automatically broadcasts events for every CRUD operation. Clients connect over WebSocket or SSE and receive filtered updates in real time.

## How it works

When a record is created, updated, or deleted, Zork fires an event through the realtime broker. Connected clients receive the event if they pass the access control check for that collection.
