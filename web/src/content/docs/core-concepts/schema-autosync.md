...
For production deployments, the recommended approach is to use explicit migrations (run `zork migrate`) and rely on auto-sync only for convenience during local development.
