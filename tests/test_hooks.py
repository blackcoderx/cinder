...
@pytest.mark.asyncio
async def test_runner_propagates_zork_error():
    r = HookRegistry()
    runner = HookRunner(r)

    def boom(p, c):
        raise ZorkError(403, "nope")
...
@pytest.mark.asyncio
async def test_before_create_zork_error_aborts(store_with_posts):
    store, posts = store_with_posts

    def veto(data, ctx):
        raise ZorkError(403, "no")
...
async def test_custom_collection_event(store_with_posts):
    ...
    await posts.fire("payment_confirmed", {"id": 1}, ZorkContext.system())
...
async def test_fire_with_no_handlers_is_noop(store_with_posts):
    ...
    # should not raise
    await posts.fire("unregistered_event", "payload", ZorkContext.system())
...
async def test_app_hooks_on_and_fire():
    ...
    await app.hooks.fire("fraud:detected", {"score": 0.95}, ZorkContext.system())
...
def test_app_startup_fires_via_testclient(tmp_path):
    ...
    app = Zork(database=str(tmp_path / "t.db"))
    startup_calls = []
    shutdown_calls = []
...
async def test_app_level_handler_fires_collection_event(tmp_path):
    ...
    app = Zork(database=str(tmp_path / "u.db"))
...
async def test_pre_registered_handlers_migrate_on_bind(tmp_path):
    ...
    app = Zork(database=str(tmp_path / "m.db"))
...
@pytest.mark.asyncio
async def test_zork_on_shorthand():
    app = Zork(database=":memory:")
    calls = []
    ...
    await app.hooks.fire("fraud:detected", {"x": 1}, ZorkContext.system())
    assert calls == [{"x": 1}]
