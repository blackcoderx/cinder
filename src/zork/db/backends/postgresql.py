...
logger = logging.getLogger("zork.db.backends.postgresql")


class PostgreSQLBackend(DatabaseBackend):
    """PostgreSQL database backend using asyncpg connection pool.

    Install: pip install zork[postgres]   (asyncpg>=0.29.0)

    Parameter style: callers write '?' — this backend converts to $1, $2, ...
    Pool size is configurable via constructor args or env vars:
        ZORK_DB_POOL_MIN  (default: 1)
        ZORK_DB_POOL_MAX  (default: 10)
        ZORK_DB_POOL_TIMEOUT   (default: 30, seconds)
        ZORK_DB_CONNECT_TIMEOUT (default: 10, seconds)
    ...
    def __init__(
        ...
        self._url = url
        self._min_size = min_size or int(os.getenv("ZORK_DB_POOL_MIN", "1"))
        self._max_size = max_size or int(os.getenv("ZORK_DB_POOL_MAX", "10"))
    ...
    async def connect(self) -> None:
        try:
            ...
            raise ImportError(
                ...
                "Install it with: pip install zork[postgres]"
            ) from exc

        connect_timeout = float(os.getenv("ZORK_DB_CONNECT_TIMEOUT", "10"))
        ...
        try:
            kwargs: dict = {
                ...
                "timeout": float(os.getenv("ZORK_DB_POOL_TIMEOUT", "30")),
