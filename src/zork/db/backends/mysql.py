...
logger = logging.getLogger("zork.db.backends.mysql")


class MySQLBackend(DatabaseBackend):
    """MySQL database backend using aiomysql connection pool.

    Install: pip install zork[mysql]   (aiomysql>=0.2.0)
    ...
    def _rewrite_ddl(sql: str) -> str:
        ...
        MySQL requires a length prefix for TEXT primary keys. Since ZORK
    ...
    async def connect(self) -> None:
        try:
            ...
            raise ImportError(
                ...
                "Install it with: pip install zork[mysql]"
            ) from exc

        connect_timeout = int(os.getenv("ZORK_DB_CONNECT_TIMEOUT", "10"))
