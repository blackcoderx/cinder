...
from zork.db.backends.base import DatabaseBackend
...
class Database:
    ...
    Environment variables override the programmatic value:
        ZORK_DATABASE_URL   — highest priority (Zork-specific)
        DATABASE_URL          — second priority (standard PaaS convention)

    Pool size (PostgreSQL / MySQL):
        ZORK_DB_POOL_MIN    — minimum connections (default: 1)
        ZORK_DB_POOL_MAX    — maximum connections (default: 10)
        ZORK_DB_POOL_TIMEOUT    — seconds to wait for a free connection (default: 30)
        ZORK_DB_CONNECT_TIMEOUT — seconds to open a new connection (default: 10)
    """
