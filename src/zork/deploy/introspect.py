"""Inspect a Zork app instance to build a deployment profile."""
...
def introspect(app_path: str) -> AppProfile:
    """Load the Zork app at *app_path* and return an :class:`AppProfile`."""
    ...
    # Find the Zork instance and its variable name
    zork_app: Zork | None = None
    var_name = "app"
    ...
    if zork_app is None:
        raise RuntimeError(f"No Zork instance found in {app_path}")

    # --- Detect database type ---
    import os

    db_url = (
        os.getenv("ZORK_DATABASE_URL") or os.getenv("DATABASE_URL") or zork_app.database
    )
    ...
    if not needs_redis:
        ...
        if isinstance(zork_app.cache._backend, RedisCacheBackend):
            needs_redis = True
        elif isinstance(zork_app.rate_limit._backend, RedisRateLimitBackend):
            needs_redis = True

    # --- Auth, S3, Email ---
    needs_auth = zork_app._auth is not None
    needs_s3 = zork_app._storage_backend is not None
    needs_email = zork_app.email._backend is not None
