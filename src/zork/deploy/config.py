"""Generate zork.toml deployment configuration."""
...
def generate_zork_toml(profile: AppProfile, platform: str) -> str:
    """Return the content of a ``zork.toml`` file."""
    db_type = (
        "postgresql"
        if profile.needs_postgres
        else "mysql"
        if profile.needs_mysql
        else "sqlite"
    )
